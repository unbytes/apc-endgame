...
from utils.functions import get_column, get_rows_without_nan
...
def join_countries_box_office_per_movie(dataframe):
  ...
  for movie_row in movies_rows:
    region_box_office = movie_row[1:]                                           # Retira o 1º elemento, ou seja, o nome do filme

    movie_region_box_office = sum(region_box_office)                            # Soma as bilheterias do respectivo filme em uma região do mundo
    movies_box_office.append(movie_region_box_office)
  return movies_box_office

asia_pacific = join_countries_box_office_per_movie(asia_pacific_box_office)     # Agrupa a bilheteria dos países de uma região do mundo para cada filme
europe = join_countries_box_office_per_movie(europe_box_office)
middle_east_and_africa = join_countries_box_office_per_movie(middle_east_and_africa_box_office)

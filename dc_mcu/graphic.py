...
def create_comparative_graphic_column_based(first_df, second_df, first_name, second_name, column_title, graphic_title):
  ...
  def count_values_appearance(values, column):
    ...
    return count                                                                # Retorna uma lista com a contagem
  ...
  graphic_figure = go.Figure([
      go.Bar(name=first_name, x=values_in_common, y=first_count, marker={'color': '#4C9B82'}),
      go.Bar(name=second_name, x=values_in_common, y=second_count, marker={'color': '#0F8BAE'})
  ])
...
hair_graphic = create_comparative_graphic_column_based(dc_wikia_dataframe, mcu_wikia_dataframe, 'DC', 'MCU', HAIR_COLUMN_TITLE, 'Cor de cabelo: Personagens da Marvel e DC')
eye_graphic = create_comparative_graphic_column_based(dc_wikia_dataframe, mcu_wikia_dataframe, 'DC', 'MCU', EYE_COLUMN_TITLE, 'Cor dos olhos: Personagens Marvel e DC')
